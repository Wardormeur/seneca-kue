...
module.exports = function Kue (role, seneca, options) {
  ...
  seneca.add({role: role, cmd: 'work'}, function (args, done) {
    ...
    var concurrency = args.params && args.params.concurrency ? args.params.concurrency : 1
    console.log('concurrency is set to: ' + concurrency)
    queue.process(queueName, concurrency, function (job, cb) {
